...
def Img2Nt(img):
    ...
    #master_forward='CGTCGTCGTCCCCTCAAACT'
	#master_reverse='GCTGAAAAGGTGGCATCAAT'
...
def traverse_folder(folder_path):
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            ...
            print(file_path)  # 在这里可以对文件路径进行处理或执行其他操作
            img=Image.open(file_path).convert(mode='1').transpose(method=Image.TRANSPOSE)
            ...
            print('>' + file_path[:file_path.rfind('.')] + '_' + str(seed))
            print(finalSeq)
            print()
